from storage import MessageStore, MemMessageStore, BotStore, MemBotStore, UserStore, MemUserStore, ChatStore, MemChatStore
...
class ChatCoreService:
    ...
    def get(self, chatId: int) -> ChatEntity:
        ...
        # assemble
        return ChatEntity(id=chat.id, userId1=user.id, userName1=user.name, userId2=bot.id, userName2=bot.name, chatHistory=messages)
...
class ChatbotMessageSendService:
    ...
    def sendMsg(self, chatId: int, msgContent: str) -> str:      
        ...
        # append new msg
        additionalMsg = Message(chatId=chatId, sender=chatEntity.userName1, message=msgContent)
        chatEntity.chatHistory.append(additionalMsg)

        # send msg 
        req = ChatRequest(user_name=chatEntity.userName1,bot_name=chatEntity.userName2,chat_history=chatEntity.chatHistory)
        botOutput = self.chatClient.send(req)

        # persist the chat
        self.chatService.persistNewMsgs([additionalMsg, Message(chatId=chatEntity.id, sender=chatEntity.userName2,message=botOutput)])
